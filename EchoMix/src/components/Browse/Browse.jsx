...
function Browse() {
  ...
  function fetchAllSongsByAlphabet() {
    const letters = "abcdefghijklmnopqrstuvwxyz".split("");
    const allResults = [];

    Promise.all(
      letters.map((letter) =>
        fetch(`http://localhost:8080/song/search?keyword=${letter}`)
          .then((res) => res.json())
          .then((data) => {
      ...
      const uniqueSongs = Array.from(
        new Map(allResults.map((s) => [s.songFile, s])).values()
      );
  ...
  function handleSearchChange(e) {
    ...
    fetch(`http://localhost:8080/song/search?keyword=${value}`)
      .then((res) => res.json())
      .then((data) => {
        setSongList(data);
      })
    ...
    return ( 
      ...
      >
        <img
          ...
          alt={song.songName}
          className="browse-songImage"
        />
        <div className="browse-songDetails">
          <p className="browse-songName">{song.songName}</p>
          <p className="browse-artistName">{song.artistName}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="browse-container">
        <NavBar className="browse-navBar" />
        <div className="browse-mainContent">
          <div className="browse-profileBarAndList">
            <div className="browse-profileBar">
              <div className="browse-searchBtnContainer">
                <img
                  src=""
                  alt=""
                />
            ...
            {songList.length > 0 ? (<div className="browse-SongList">
              {songList.map((s, i) => {
                return createSongCard(s,i);
              })}
            </div>) 
            : 
            <div className="browse-noSearch">
              No songs found...
              </div>}
          ...
          {song ? (
            <div className="browse-playingContainer">
              <div className="browse-nowImage">
                <p>Now Playing...</p>
              </div>
              <div className="browse-nowPlayingContainer">
                <img src={getArtistImgSrc(song.artistName)} alt="" />
                <div className="browse-playingTitleContainer">
                  <p className="browse-playingSongName">{song.songName}</p>
                  <p className="browse-playingArtistName">{song.artistName}</p>
                </div>
                <audio
                  className="browse-audioBar"
                  controls
                  preload="metadata"
                  loop
            ...
            <div className="browse-playingContainer">
              ...
              <div className="browse-nowPlayingContainer">
                ...
                />
                <div className="browse-playingTitleContainer">
                  <p className="browse-playingSongName" hidden>
                    SongName
                  </p>
                  <p className="browse-playingArtistName" hidden>
                    ArtistName
                  </p>
                </div>
                <audio

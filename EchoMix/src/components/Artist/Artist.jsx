...
function Artist() {
  ...
  async function getArtist() {
    try {
      const response = await axios.get(
        `http://localhost:8080/artist/search/${artistName}`
      );
  ...
  async function getSongList() {
    try {
      const response = await axios.get(
        `http://localhost:8080/song/getArtistSongs/${artistName}`
      );
      if (response.status === 200) {
        setSongList(response.data);
      } else {
        console.error("Error fetching song list:", response.status);
      }
    } catch (error) {
  ...
  function displayArtistSong(src) {
    return (
      <div
        className="Artist-SongCard"
        onClick={() => {
          handleSongClick(src);
          setSong(src);
        }}
      >
        <div className="Artist-SongCardImg">
          <img
          ...
          <div className="Artist-songTitle">
            <p>{src.songName}</p>
          </div>
  ...
  return (
    <>
      <div className="Artist-container">
        <NavBar className="Artist-navBar" />
        <div className="Artist-mainContent">
          <div className="Artist-profileBarAndList">
            <div className="Artist-img">
              <img src={getArtistImgSrc(artistName)} alt={artistName} />
            </div>
            <p className="Artist-name">{artistName}</p>
            <p className="Artist-joinDate">
              Joined on :
              {src && src.date
                ? " " +
            ...
            <p className="Artist-songsBy"> Songs by this artist:</p>
            <div className="Artist-songList">
              {songList.length > 0 ? (
                songList.map((src, index) => {
                  return (
                    <React.Fragment key={index}>
                      {displayArtistSong(src)}
                    </React.Fragment>
                  );
                })
              ) : (
                <p>No songs by this artist.</p>
              )}
            </div>
          </div>
          {song ? (
            <div className="Artist-playingContainer">
              <div className="Artist-nowImage">
                <p>Now Playing...</p>
              </div>
              <div className="Artist-nowPlayingContainer">
                <img src={getArtistImgSrc(song.artistName)} alt="" />
                <div className="Artist-playingTitleContainer">
                  <p className="Artist-playingSongName">{song.songName}</p>
                  <p className="Artist-playingArtistName">{song.artistName}</p>
                </div>
                <audio
                  className="Artist-audioBar"
                  controls
                  preload="metadata"
                  loop
            ...
            <div className="Artist-playingContainer">
              ...
              <div className="Artist-nowPlayingContainer">
                ...
                />
                <div className="Artist-playingTitleContainer">
                  <p className="Artist-playingSongName" hidden>
                    SongName
                  </p>
                  <p className="Artist-playingArtistName" hidden>
                    ArtistName
                  </p>
                </div>
                <audio
